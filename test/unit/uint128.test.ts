...
test("Uint128 addition operations", () => {
  const a = Uint128.makeUint128FromPayload(Buffer.from("00000000000000000000000000000005", "hex"));
  const b = Uint128.makeUint128FromPayload(Buffer.from("00000000000000000000000000000003", "hex"));
  const result = a.add(b);
  const expected = Uint128.makeUint128FromPayload(Buffer.from("00000000000000000000000000000008", "hex"));
...
test("Uint128 addition with carry", () => {
  ...
  const b = Uint128.makeUint128FromPayload(Buffer.from("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF", "hex"));
  const one = Uint128.one();
...
test("Uint128 subtraction operations", () => {
  const a = Uint128.makeUint128FromPayload(Buffer.from("00000000000000000000000000000008", "hex"));
  const b = Uint128.makeUint128FromPayload(Buffer.from("00000000000000000000000000000003", "hex"));
  const result = a.sub(b);
  const expected = Uint128.makeUint128FromPayload(Buffer.from("00000000000000000000000000000005", "hex"));
...
test("Uint128 buffer round-trip", () => {
  ...
  for (const buffer of testCases) {
    ...
    assert.ok(buffer.equals(roundTrip), `Buffer round-trip failed for ${buffer.toString("hex")}`);
  }
});
...
test("Uint128 invalid buffer length", () => {
  ...
  for (const buffer of invalidBuffers) {
    assert.throws(() => Uint128.makeUint128FromPayload(buffer), `Should throw for buffer length ${buffer.length}`);
  }
});
...
test("Uint128 KSUID payload conversion", () => {
  ...
  // Test static method
  const uint128 = Uint128.uint128Payload(Buffer.concat([Buffer.alloc(4), testBuffer]));
  assert.ok(uint128.bytes().equals(testBuffer));
});

test("Uint128 KSUID generation", () => {
  const uint128 = Uint128.makeUint128FromPayload(Buffer.from("0123456789ABCDEF0123456789ABCDEF", "hex"));
...
test("Uint128 high precision arithmetic", () => {
  ...
  const highValue = Uint128.makeUint128FromPayload(Buffer.from("FFFFFFFFFFFFFFF000000000000000FF", "hex"));
  const increment = Uint128.makeUint128FromPayload(Buffer.from("00000000000000000000000000000001", "hex"));

  const result = highValue.add(increment);
  const expected = Uint128.makeUint128FromPayload(Buffer.from("FFFFFFFFFFFFFFF00000000000000100", "hex"));
...
test("Uint128 carry propagation edge cases", () => {
  ...
  const nearCarry = Uint128.makeUint128FromPayload(Buffer.from("000000000000000FFFFFFFFFFFFFFFFF", "hex"));
  const one = Uint128.one();

  const result = nearCarry.add(one);
  const expected = Uint128.makeUint128FromPayload(Buffer.from("00000000000000100000000000000000", "hex"));
...
test("Uint128 performance with large operations", () => {
  ...
  const end = Date.now();
  assert.ok(end - start < 100, "10k increment operations should complete within 100ms");

  const expected = Uint128.makeUint128FromPayload(Buffer.from("00000000000000000000000000002710", "hex"));
